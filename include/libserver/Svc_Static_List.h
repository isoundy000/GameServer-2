...
public:
    ...
    virtual std::vector<NODE_TYPE> erase_all_used(void) {
        std::vector<NODE_TYPE> used_list;
        while (used_head().next_ != this->used_list_head_idx)
        {
            Node &node = data_list_[used_head().next_];
            used_list.push_back(node.node_);
            this->erase_svc(node.idx_);
        }
        return used_list;
    }
	...
	int erase_svc(int idx) {
		...
		if (! node.is_used_) {
			LOG_USER_INFO("cid = %d is a free node", idx);
			return -1;
		}
...
private:
	...
	void init(void) {
		erase_interval_ = Time_Value(8 * 60, 0);
